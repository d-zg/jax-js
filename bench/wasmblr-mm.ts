...
// ─── jax-js JIT codegen benchmark ─────────────────────────────────────────
// Uses jax-js's public API to compile a matmul, then benchmarks the compiled
// kernel with the same methodology as the hand-tuned versions above.
...

{
  ...
  if (!devices.includes("wasm")) {
    console.log("\n  jax-js codegen: skipped (wasm not available)");
  } else {
    defaultDevice("wasm");

    const a = random.uniform(random.key(0), [M, N]);
    const b = random.uniform(random.key(1), [N, K]);

    const f = jit((a: np.Array, b: np.Array) => np.matmul(a, b));

    // Warmup: compile + run once
    const warmupResult = f(a.ref, b.ref);
    await warmupResult.blockUntilReady();
    warmupResult.dispose();

    // Warmup iterations
    for (let i = 0; i < WARMUP; i++) {
      const r = f(a.ref, b.ref);
    ...
    console.log(
      `\n  jax-js codegen:    ${computeGflops(M, N, K, medianMs).toFixed(2)} gflops (median ${medianMs.toFixed(2)}ms, best ${computeGflops(M, N, K, minMs).toFixed(2)} gflops)`,
    );
